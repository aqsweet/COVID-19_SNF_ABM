...
ABM_SNFdynamics_func <- function(
  ...
  # Version 3.05 Changelog====
  #* 7/26/24
  #* Tweaks to transmission model for SNF and hospital 
  #* Updated how policies are implemented
...
# Bug Fixes
  ...
  verbose = 0,
  iter = copy(iter),
  mandatory_sickDays = TRUE, #is it mandatory to take sick leave when symptomatic?
  ...
  community_infection_prevalence = (71/100000)*5, #community infection rate; 
  ...
  #contact with in a typical day
  other_contacts = 4.59, #Number of non-nurse/non-resident contacts
  ratio_other_contacts = 2.89,
  patient_contacts = 6, #Number of other patients a hospital patient has direct
  #contact with in a typical day
  hosp_worker_contacts = 6,
  asymptomatic_rate = 0.4632, 
  transmission_prob = 0.14,
  asympto_incubation = 5.47,
  sympto_incubation = 2.63,
  ...
  vaccination_policy_rates = c(no_policy = 0.3, policy = 0.9), #vaccination 
  ...
  pto = pto_rate, #The PTO policy for this simulation [0,1,2]
  vax = vaccination_policy, #vaccination policy for this simulation [0,1]
  ...
  action_sub_model_vector <- c("death", "upkeep_staff_counts", 
                               "Transmission", "hospital_admission", 
                               "hospital_discharge", "recovery", 
                               "retire", "SNF_admission", "vaccination", 
                               "SNF_discharge")#for record-keeping purposes we'll 
  ...
  SNF_pop[ , `:=` (died = NA_integer_, birthday = (sample(1:365, nrow(SNF_pop), replace = TRUE)), time_in_state_SIRV = 0, time_in_state_RAH = 0)] #create columns in SNF_pop
  community_pop[ , `:=` (died = NA_integer_, birthday = sample(1:365, nrow(community_pop), replace = TRUE), time_in_state_SIRV = 0, time_in_state_RAH = 0, PTO_1 = as.numeric(rpois(n = nrow(community_pop), lambda = initial_PTO_distribution_lambda)), PTO_2 = 0, PTO_3 = 0, retired = 0, replaced = 0)] #create columns in community_pop
  ...
  total_population[ , `:=` (current_location_id = dwelling_id, population = c(rep("SNF", nrow(SNF_pop)), rep("community", nrow(community_pop))), state_SIRV = "S", symptoms = "0",  state_RAH = "R", H_id = NA_character_, length_hospital_stay = NA_integer_, state_change_SIRV = as.character(0), state_change_RAH = as.character(0), n_times_infected = 0, host_clearance_time = NA_integer_, infection_time = NA_integer_, n_doses_vaccine = 0, time_of_last_vaccine = NA_integer_, prevalence_threshold_to_vaccinate = rbeta(.N, shape1 = unname(prevalence_threshold_to_vaccinate_beta_distribution_parameters[1]), shape2 = unname(prevalence_threshold_to_vaccinate_beta_distribution_parameters[2]))*prevalence_threshold_multiplier_population)]
  ...
  total_population[H_Assigned, state_change_RAH := "H"]
  
  H_transition_vec <- total_population[state_change_RAH == "H", which = TRUE]
  
  total_population[H_transition_vec, length_hospital_stay := rpois( n = length(H_transition_vec), lambda = days_hospitalized_lambda)]
  
  SNF_H_transition_timestep_vec <- total_population[population == "SNF" & state_change_RAH == "H", which = TRUE]
  count_snf <- 0
    ...
    for(b in 1:ncol(hospital_preference_vec)){
      while(assigned_H == 0){
        ...
        if(is.na(beds_avail) == FALSE | beds_avail > 0){
          ...
          total_population[a, `:=` (state_RAH = new_state, H_id = hosp_of_choice, time_in_state_RAH = 0, current_location_id = hosp_of_choice)]
  ...
  Comm_H_transition_timestep_vec <- total_population[population == "community" & state_change_RAH == "H", which = TRUE]
  count_comm <- 0
    ...
    for(b in 1:ncol(hospital_preference_vec)) {
      while(assigned_H == 0){
        ...
        if(count_col > ncol(hospital_preference_vec)){ #if count_col exceeds the number of columns in the preference frame, then it indicates that there are no available beds in hospitals. In this case, SNF residents will be assigned to the "Awaiting care" (A) state specific to their SNF.
          ...
          total_population[a, `:=` (state_RAH = new_state, time_in_state_RAH = 0)]
  ...
  for(tick in 1:n_timesteps){
    ...
    alive_table[ , `:=` (state_change_SIRV = 0, state_change_RAH = 0, time_in_state_SIRV = time_in_state_SIRV + 1, time_in_state_RAH = time_in_state_RAH + 1, infected_this_timestep = 0, time_of_last_vaccine = time_of_last_vaccine + 1)] #resets variables for the simulation, adds a day to time in each state, updates host clearance time
    ...
    #1) death - kill individuals and remove them from the population
    ...
    #-3) Transmission - Some people get infected.
    #-4) hospital_admission - Some people move to the "H" state (or "A" if all hospitals are full).
    #-5) recovery - People will spontaneously clear the infection, and may have an elevated probability of doing so when hospitalized.
    #-6) hospital_discharge - People leave the hospital after hitting their required number of days.
    #-7) retire - People have the opportunity to retire from their current workplace
    #-8) SNF_admission - People may move to an SNF.
    #-9) vaccination - People may seek a dose of vaccine.
    ...
    for(random_pull in 1:sub_model_count){ 
      ...
      if(sub_m == "death"){
        ...
        alive_table[, mortality_probability := baseline_mortality_lambda * exp(
          get_age_mod(age, mortality_AFT_hazard_betas) + 
            get_state_mod(state_SIRV, state_RAH, mortality_AFT_hazard_betas) + 
        ...
        died_in_hospital_table <- alive_table[died_this_timestep == 1 & H_id %in% hospital_ids,] #pull the RAH state for individuals who died
      ...
      #3 Sub-Model Transmission====
      #      
      
      if(sub_m == "Transmission"){  
        if(verbose == 2){print(paste0("Sub Model ", sub_m, " iteration ", tick, " beginning"))}
        ...
        infectious_hosts <- infected_hosts[(symptoms == "Y" & time_in_state_SIRV >= sympto_incubation)|(symptoms == "N" & time_in_state_SIRV >= asympto_incubation) , ] #removes individuals in the latency period
        
        comm <- alive_table[population == "community" & (state_RAH == "A" | state_RAH == "R"), which = TRUE] #total community not in the hospital
        ...
        result <- calculate_hospital_infection_probability(hospital_data, alive_table, infectious_hosts, patient_contacts, hosp_worker_contacts, community_infection_prevalence, mandatory_sickDays, transmission_prob, other_contacts, ratio_other_contacts)
        ...
        result <- calculate_snf_infection_probability(SNF_data, alive_table, infectious_hosts, resident_contacts, worker_contacts, community_infection_prevalence, mandatory_sickDays, transmission_prob, other_contacts, ratio_other_contacts)
        SNF_data <- result[[1]]
        alive_table <- result[[2]]
        ...
        infected_comm <- sample(alive_table[comm][state_SIRV != "I", which = TRUE], num_comm * community_infection_prevalence)
      ...
      } # end sub_m == "Transmission"
      
      #4 Sub-Model hospital_admission====
      
      if(sub_m == "hospital_admission"){
        ...
        alive_table[state_RAH == "A", hospitalization_probability := 1] #makes sure hospitalization probability is 1 for those already in "A" state
        
        #decide who will attempt to be hospitalized this time step
        hospitalized_this_timestep<- alive_table[hospitalization_probability > runif(nrow(alive_table)) & state_change_RAH == 0, which = TRUE]
        ...
        alive_table[hospitalized_this_timestep][is.na(H_id) & state_RAH != "A", `:=` (state_change_RAH = "A", state_RAH = "A", time_in_state_RAH = 0)] #updates RAH variables for those not assigned to hospitals (not already in A state)
        
        alive_table[!is.na(H_id), `:=` (state_change_RAH = "H", state_RAH = paste0("H_",H_id), current_location_id = H_id, time_in_state_RAH = 0)] #updates RAH variables for those assigned to hospitals
      ...
      } #end of hospital_admission sub model
      
      
      #5 Sub-Model hospital_discharge====
      # 
      if(sub_m == "hospital_discharge"){
        
        if(verbose == 2){print(paste0("Sub Model ", sub_m, " iteration ", tick, " beginning."))}
        
        
        leaving_hospital_this_timestep <- alive_table[grepl("H", state_RAH) & state_change_RAH == 0 & symptoms != "Y" & length_hospital_stay == time_in_state_RAH, which = TRUE] #we pull a list of individuals who have been in hospital for the predetermined stay length (and who are NOT symptomatic with COVID)
        
        if(length(leaving_hospital_this_timestep) > 0){
          alive_table[leaving_hospital_this_timestep, `:=` (state_RAH ="R", state_change_RAH = "R", H_id = NA, current_location_id = dwelling_id, time_in_state_RAH = 0, length_hospital_stay = NA)]#update individuals to move back to their normal community dwelling (SNF or home)
      ...
      }# end sub_m == "hospital_discharge"
      
      #     #6 Sub-Model recovery====
      # 
      if(sub_m == "recovery"){
        
        if(verbose == 2){print(paste0("Sub Model ", sub_m, " iteration ", tick, " beginning."))}
        
        recovering_this_timestep <- alive_table[state_SIRV == "I" & time_in_state_SIRV >= host_clearance_time, which = TRUE] #find individuals who have been infected for their full clearance time [see Transmission submodel for how host clearance time is assigned]
     ...
     if(sub_m == "retire"){
      ...
      #     #8 Sub-Model SNF_admission====
      # 
      if(sub_m == "SNF_admission"){
        
        if(verbose == 2){print(paste0("Sub Model ", sub_m, " iteration ", tick, " beginning."))}
        
        medicare_move <- alive_table[symptoms != "Y" & population == "community" & age >= 65, which = TRUE] #finds individuals eligible to move
        
        needs_to_move <- alive_table[medicare_move][baseline_medicare_lambda*exp(medicare_age_beta*age + 5.25*ifelse(state_RAH != "A" & state_RAH != "R",1,0)) > runif(length(medicare_move)), which = TRUE] #finds individuals who need to move this timestep
      ...
      }}# end sub_m == "SNF_admission"
      
      #     #9 Sub-Model SNF_discharge====
      # 
      if(sub_m == "SNF_discharge"){
      ...
      } # end sub_m == "SNF_discharge"
      #
      #`**************************************************************************`
      #
      #     #10 Sub-Model vaccination====
      # 
      if(sub_m == "vaccination"){
